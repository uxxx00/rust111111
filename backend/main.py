...
class WebSocketLogHandler(logging.Handler):
    def emit(self, record):
        try:
            ...
            if '"GET /' in log_entry or '"POST /' in log_entry or '"OPTIONS /' in log_entry:
                return
            socketio.emit('bot_log', {'message': log_entry})
        except Exception:
            pass
...
class BotManager:
    ...
    def start_bot(
        ...
        if stability_mode and not is_dev_request and normalized_status not in ALLOWED_STABILITY_SUBSCRIPTIONS:
            logger.warning("Stability mode requested without valid subscription, but unlocked for free")
...
def handle_start_bot(data):
    ...
    try:
        ...
        enable_chat = data.get('enableChat', False)
        if enable_chat and CHAT_AVAILABLE:
            auth_token = data.get('authToken', '')
            msgs_per_min = data.get('messagesPerMinute', 1)
            # interval = 60 seconds / msgs_per_min
            min_interval = max(3, int(60 / msgs_per_min))
            chat_service.start_kick_chat(
                channel_name=channel_name,
                auth_token=auth_token,
                response_chance=1.0, # Always respond if within rate limit
...
@app.route('/viewer_count/<channel_name>')
def viewer_count(channel_name):
    import requests
    try:
        resp = requests.get(f'https://kick.com/api/v1/channels/{channel_name}')
        if resp.status_code == 200:
            return {'viewer_count': resp.json().get('livestream', {}).get('viewer_count', 0)}
    except:
        pass
    return {'viewer_count': 0}

@app.route('/verify_tokens', methods=['POST'])
def verify_tokens():
    import requests
