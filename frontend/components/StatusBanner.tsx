...
export const StatusBanner = ({ status }: StatusBannerProps) => {
  ...
  useEffect(() => {
    if (status.state === "starting" && status.startup_progress > 0 && status.startup_progress < 100) {
      if (!startTime) {
        ...
        if (progress > 5) {
          ...
          setEstimatedSeconds(Math.max(0, Math.ceil(remaining / 1000)));
        }
      }
  ...
  }, [status.startup_progress, status.state]);
  const getTextColor = () => {
    switch (status.state) {
      case "error":
        return "text-red-500";
                  ...
                  <div className="animate-fade-in">
                    <div className="flex justify-between text-sm mb-1.5">
                      <span className="font-medium flex items-center">
                        Starting Bot
                        {estimatedSeconds !== null && status.startup_progress < 100 && (
                          <span className="ml-2 text-[11px] text-zinc-500 font-normal tracking-wide">
                            (est. {estimatedSeconds < 60 ? `${estimatedSeconds}s` : `${Math.floor(estimatedSeconds/60)}m ${estimatedSeconds%60}s`} left)
                          </span>
                        )}
                      </span>
                      <span className="font-medium">
