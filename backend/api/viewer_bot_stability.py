...
class ViewerBot_Stability:
    ...
    def get_websocket_token(self):
        ...
        try:
            ...
            if hasattr(self, 'token_cache') and self.token_cache and hasattr(self, 'token_cache_time'):
                # Tokens are valid for a while, let's cache for 5 minutes
                if current_time - self.token_cache_time < 300:
                    logging.debug("Using cached WebSocket token")
                    return self.token_cache
            
            # Add a small random delay to prevent thunderous herd if multiple bots are starting
            ...
            if hasattr(self, 'token_lock') and self.token_lock.locked():
                try:
    ...
    def stop(self):
        ...
        logging.debug("Bot stopped and all tasks cleaned up")
    ...
    async def _websocket_worker(self, proxy_data):
        ...
        while not self.should_stop and retry_count < self.max_retry_attempts:
            try:
                # Get a FRESH token for THIS connection asynchronously
                logging.info(f"[{connection_id}] Getting WebSocket token...")
                token = await asyncio.to_thread(self.get_websocket_token)
                if not token:
                ...
                async with websockets.connect(ws_url, **connect_kwargs) as websocket:
                    ...
                    try:
                        ...
                        # Matrix effect output
                        print(f"\033[92m[+] SUCCESS: Viewer joined channel {self.channel_name} via {proxy_type.upper()} ({proxy_address.split(':')[0]})\033[0m")
    ...
    async def async_main(self):
        self.update_status('running', 'Bot is highly optimized with Asyncio for massive concurrency', 
        ...
        while not self.should_stop:
            ...
            self.active_threads = active_connections
            
            # Start new connections if we are below the thread limit
            if active_connections < self.nb_of_threads and len(self.all_proxies) > 0:
                needed = int(self.nb_of_threads) - active_connections
                for _ in range(needed):
                    if self.should_stop:
                        break
                    proxy_data = self.all_proxies[random.randrange(len(self.all_proxies))]
                    task = asyncio.create_task(self._websocket_worker(proxy_data))
                    tasks.add(task)
